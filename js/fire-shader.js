(function initFluidPointerBackground(){
  ...
  const pointer = { x: window.innerWidth * 0.64, y: window.innerHeight * 0.34, targetX: window.innerWidth * 0.64, targetY: window.innerHeight * 0.34 };
  ...
  function render(now){
    ...
    for (let i = 0; i < trail.length; i++) {
      packedTrail[i * 2] = trail[i].x / window.innerWidth;
      packedTrail[i * 2 + 1] = 1 - trail[i].y / window.innerHeight;
    }
